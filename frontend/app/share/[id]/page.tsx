...
import { useShare } from "@/hooks/queries/useShare";
...
export default function SharePage({ params }: SharePageProps) {
  const { id } = use(params);
  const { data: shareData, isLoading, error } = useShare(id);
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  const [mapFocusPlace, setMapFocusPlace] = useState<Place | null>(null);

  useEffect(() => {
    if (!shareData?.categoryResults || Object.keys(shareData.categoryResults).length === 0) {
      return;
    }

    const preferred =
      shareData.selectedCategoryCode && shareData.categoryResults[shareData.selectedCategoryCode]
        ? shareData.selectedCategoryCode
        : shareData.used?.category && shareData.categoryResults[shareData.used.category]
          ? shareData.used.category
          : Object.keys(shareData.categoryResults)[0];
  ...
  if (error || !shareData) {
    const errorMessage = error instanceof Error ? error.message : "공유 데이터를 불러올 수 없습니다.";
    return (
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        <div className="text-center py-20">
          <div className="text-6xl mb-4">😕</div>
          <h1 className="text-2xl font-bold text-slate-800 mb-2">공유 데이터를 찾을 수 없습니다</h1>
          <p className="text-slate-600 mb-6">{errorMessage || "링크가 만료되었거나 존재하지 않습니다."}</p>
  ...
  const categoryResult = hasCategoryResults && selectedCategory ? shareData.categoryResults![selectedCategory] : null;
